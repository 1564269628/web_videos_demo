...
class Author:
    ...
    def public(self) -> dict[str, Any]:
        return {
            "id": self.id, "folderName": self.folder_name, "name": self.name, "uid": self.uid,
            "signature": self.signature, "videoCount": self.video_count, "downloadedCount": len(self.works),
            "followerCount": self.follower_count, "likedCount": self.liked_count, "collectCount": self.collect_count,
            "followerCountLabel": compact_count(self.follower_count), "likedCountLabel": compact_count(self.liked_count),
            "collectCountLabel": compact_count(self.collect_count), "avatarUrl": f"/avatar/{self.id}",
        }
...
class Work:
    ...
    def public(self, author: Author) -> dict[str, Any]:
        return {
            "id": self.id, "sourceId": self.source_id, "index": self.index, "title": self.title,
            "description": self.description, "duration": self.duration, "width": self.width, "height": self.height,
            "playCount": self.play_count, "likeCount": self.like_count, "commentCount": self.comment_count,
            "collectCount": self.collect_count, "playCountLabel": compact_count(self.play_count),
            "likeCountLabel": compact_count(self.like_count), "commentCountLabel": compact_count(self.comment_count),
            "collectCountLabel": compact_count(self.collect_count), "releaseDate": self.release_date, "tags": self.tags,
            "coverUrl": f"/cover/{self.id}", "fileType": self.video.suffix.lower().lstrip("."),
            "author": {"id": author.id, "name": author.name, "uid": author.uid, "avatarUrl": f"/avatar/{author.id}"},
        }
...
class Catalog:
    ...
    def scan(self) -> None:
        ...
        works: dict[str, Work] = {}
        order: list[str] = []
        for author_dir in sorted((path for path in self.root.iterdir() if path.is_dir()), key=lambda path: path.name.lower()):
            author_json = read_json(author_dir / "author.json", {}) or {}
            works_json = read_json(author_dir / "works.json", []) or []
            works_dir = author_dir / "works"
            if author_dir.name.startswith(".") or not works_dir.is_dir() or not isinstance(works_json, list):
                continue
            fallback_name, fallback_uid = folder_identity(author_dir.name)
            author_id = file_id(author_dir)
            author = Author(
                author_id, author_dir.name, author_dir,
            ...
            for number, raw in enumerate(works_json, 1):
                ...
                by_index[index] = item
            for work_dir in sorted((path for path in works_dir.iterdir() if path.is_dir()), key=lambda path: path.name.lower()):
                ...
                source_id = first_nonempty(metadata.get("id"), raw_metadata.get("id"), work_dir.name.rsplit("_", 1)[-1])
                merged = {**(by_id.get(source_id) or by_index.get(folder_index) or {}), **raw_metadata, **metadata}
                source_id = first_nonempty(merged.get("id"), merged.get("videoId"), merged.get("vid"), source_id)
                tags: list[str] = []
                ...
                work = Work(
                    work_id, author_id, work_dir, video, find_first_file(work_dir, COVER_NAMES),
                    safe_int(merged.get("index"), folder_index or len(author.works) + 1), source_id,
                    ...
                    safe_int(merged.get("width")), safe_int(merged.get("height")),
                    ...
                    first_nonempty(merged.get("releaseDateLabel"), merged.get("releaseDate")), tags,
                )
                works[work_id] = work
        ...
        with self.lock:
            self.authors, self.works, self.work_order = authors, works, order
            self.scanned_at = time.time()

    def summary(self, ffmpeg_available: bool) -> dict[str, Any]:
        with self.lock:
            return {
                "version": APP_VERSION, "root": str(self.root), "authorCount": len(self.authors),
                "videoCount": len(self.works), "totalBytes": sum(work.video.stat().st_size for work in self.works.values()),
                "ffmpegAvailable": ffmpeg_available, "scannedAt": self.scanned_at,
            }
    ...
    def feed(self, seed: str, offset: int, limit: int) -> dict[str, Any]:
        with self.lock:
            ...
            return {"items": [self.works[item].public(self.authors[self.works[item].author_id]) for item in selected],
                    "offset": offset, "limit": limit, "total": len(ids), "hasMore": offset + len(selected) < len(ids), "seed": seed}
    ...
    def author_payload(self, author_id: str, offset: int, limit: int) -> dict[str, Any] | None:
        with self.lock:
            ...
            if not author:
                return None
            ordered = sorted((self.works[item] for item in author.works), key=lambda work: (work.index, work.title))
            selected = ordered[max(0, offset):max(0, offset) + max(1, min(limit, 200))]
            return {"author": author.public(), "items": [work.public(author) for work in selected], "offset": offset,
                    "limit": limit, "total": len(ordered), "hasMore": offset + len(selected) < len(ordered)}
